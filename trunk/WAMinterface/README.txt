...
NOTE: Most of these libraries will only compile on the WAM's computer (they require the BarrettWAM package, which depends on Xenomai/RTAI), but to use the ROS interface, all you need to compile is the ROS services, so to call the ROS services from another computer, either comment out all but the `rospack find roslib`/scripts/gensrv srv/*.srv line, or just ignore all the errors when you type make in this directory.  This package is configured for a Xenomai/Peak CAN bus system.  It will probably work in other configurations if you edit the Makefiles, but no guarantees.

To use the non-ROS interfaces, look at the READMEs in WAMinterfacelib and WAMinversekinematics.  To use the ROS interface, install ROS as described at http://pr.willowgarage.com/wiki/ROS and look at the README in the ROS directory.
...
WAMinterfacelib: 
    ...
    -a simple socket interface (can also be used without ROS, from the same or a different computer, or from any language that you can write a socket client in)
...
Makefile: recursive make for all the relevant subdirectories.
